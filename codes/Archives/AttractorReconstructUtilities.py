...
def TimeDelayReconstruct_allD(TimeSeries, tauIndex, Dimension = 10):
    ...
    # Output: 
    #     Reconstru_allD: a list of length (Dimension-1) containing points in the reconstructed [2, Dimension] dimensional state space
    length = len(TimeSeries) - (Dimension-1)*tauIndex
    Reconstru_allD = []
    for d in range(1, Dimension+1):
        Reconstru_D = np.zeros((length, d)) 
        for i in range(0, length):
            Reconstru_D[i] = TimeSeries[i:i+(d-1)*tauIndex+1:tauIndex]
        Reconstru_allD.append(Reconstru_D)
    return Reconstru_allD

def fnnCao(Reconstru_allD, PLOT=False):
    # Find the percentage of false nearst neighbor in dimension d 
    # Inputs: 
    #    Reconstru_allD: vectors in reconstructed state space
    ...
    Dimension = len(Reconstru_allD)
    E = []
    Es = []
    E1 = []
    E2 = []
    for d in range(1, Dimension):
        Reconstru_D = Reconstru_allD[d-1] 
        Reconstru_Dplus1 = Reconstru_allD[d]
        ...
        Reconstru_Dplus1_NN = Reconstru_Dplus1[ii]
        #Es_d = np.mean(abs(Reconstru_Dplus1[:,-1] - Reconstru_Dplus1_NN[:,-1]))
        R_d_plus1 = np.amax(abs(Reconstru_Dplus1 - Reconstru_Dplus1_NN), axis = 1) # Maximum norm  
