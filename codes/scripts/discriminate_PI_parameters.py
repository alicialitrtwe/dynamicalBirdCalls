...
pixels_list = [10, 15, 20] #best: 500, 20, 0.01 for 300: 10, 0.01
spread_list = [0.01] 
# 500 mean scores:  79.65817674010164 n_high_score/ntotal: 62/231 nSigP/ntotal: 220/231 for 0.01
# 500 mean scores:  79.39621194131344 n_high_score/ntotal: 57/231 nSigP/ntotal: 222/231 for 0.001
embed_dim_list = [10]
tau_list = [3]
		...
		for tau in tau_list:
			for pixels in pixels_list:
				for spread in spread_list:
					PI = np.load('TrainingData/PI_Len%s_Dim%s_Tau%s_Pix%s_Spd_%s.npy' %(segment_len, embed_dim, tau, pixels, spread))
					PI_list.append(PI)
					PI_parameter_list.append('L%sD%sT%sP%sS%s' %(segment_len, embed_dim, tau, pixels, spread))

period_list = np.load('TrainingData/period_list_500.npy')

spectrum_list = np.load('TrainingData/spectrum_list_500.npy')
fund_feat_list = np.load('TrainingData/fund_feat_list_500.npy')
...
PI_list = [PI[Ind] for PI in PI_list]
...
feature_list = PI_list
feature_name_list = PI_parameter_list
...
for i in range(len(feature_list)):
	...
	scores, p_value, n_high_score, n_counter = pairwise_logi_regression(feature_list[i], bird_name_list, nonanIndFeature=no_nan_idx, cv=True, printijScores=False)
	np.savez('%s_classi_perfrom.npz' %feature_name_list[i], scores=scores, p_value=p_value, n_high_score=n_high_score, n_counter=n_counter)
	#classi_perform = np.load('%s_classi_perform.npz' %feature_name_list[i])
	#cores = classi_perform['scores']
	#p_value = classi_perform['p_value']
	#n_high_score = classi_perform['n_high_score']
	#n_counter = classi_perform['n_counter']

	sig_idx = np.where(p_value<0.05)[0]
	no_sig_idx = np.where(p_value>=0.05)[0]
